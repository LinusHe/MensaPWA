...
def generate_notification_completion(current_date, script_dir, output_dir):
    ...
    notificationPromptFilePath = os.path.join(
        script_dir, "notification_generator", "notificationPrompt.txt"
    )
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if (
                ...
                and "title" in output["notification"]
                and "body" in output["notification"]
            ):
    ...
    with open(
        f"{output_dir}/{current_date}/notification.json", "w", encoding="utf-8"
    ) as f:
